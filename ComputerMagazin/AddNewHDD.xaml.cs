...
{
    public partial class AddNewHDD : Window
    {
        public AddNewHDD(string category)
        {
            InitializeComponent();
        ...
        private pc_HDD currentElement = new pc_HDD();
        ...
        private void AddElement_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            if (currentElement.id_HDD == 0)
            {
                currentElement.HDD = SelectName.Text;
                currentElement.price_HDD = int.Parse(Price.Text);
                ComputerMagazinEntities.GetContext().pc_HDD.Add(currentElement);
            }
            ComputerMagazinEntities.GetContext().SaveChanges();
            MessageBox.Show($"Жесткий диск HDD: {SelectName.Text}  Цена: {Price.Text}РУБ. Успешно добавлен.");
            SelectName.Clear();
            Price.Clear();
        }
